...
Shader "Unlit/ScannerDotShaderTemporary"
{
    Properties
    {
        ...
        _DotLifetime("Dot Lifetime", float) = 3
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            StructuredBuffer<float4> _AllInstancesTransformBuffer;
            ...
            v2f vert (appdata v, uint instanceID: SV_InstanceID)
            {
                ...
                float4 perDotPivotPos = _AllInstancesTransformBuffer[instanceID];
                ...
                float3 diff = perDotPivotPos.xyz - _WorldSpaceCameraPos;
                float distSq = max(dot(diff, diff), .000001);
                distSq = distSq*rsqrt(distSq);
                o.camDist = saturate(distSq/_SampleDistance);
                float scale = _DotScale;
                if(o.camDist > 0.5){
                    scale /= 2*o.camDist;
                    }
                scale *= saturate(_DotLifetime-_Time.y+perDotPivotPos.w);
                float3 worldPos = perDotPivotPos + camRight * v.vertex.x*_DotScale + camUp * v.vertex.y*scale;
